...
class RegistrationControllerTest extends ControllerTest {
    ...
    void registerUser_validRequest_returnsOk() throws Exception {
        ...
        mockMvc.perform(post("/api/auth/sign-up")
                        ...
                        .content(objectMapper.writeValueAsString(new RegistrationRequestDto(USERNAME, EMAIL, PASSWORD, Role.EMPLOYEE))))
    ...
    void registerUser_invalidEmailFormat_returnsBadRequest() throws Exception {
        mockMvc.perform(post("/api/auth/sign-up")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new RegistrationRequestDto(USERNAME, "invalid-email", PASSWORD, Role.EMPLOYEE))))
    ...
    void registerUser_invalidUsernameLength_returnsBadRequest() throws Exception {
        mockMvc.perform(post("/api/auth/sign-up")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(new RegistrationRequestDto("tu", EMAIL, PASSWORD, Role.EMPLOYEE))))
                .andExpect(status().isBadRequest())
    ...
    private RegistrationRequestDto newRegistrationRequestDto(String username, String email, String password) {
        return new RegistrationRequestDto(username, email, password , Role.EMPLOYEE);
    }
